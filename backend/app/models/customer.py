...
class Customer(Base):
    ...
    preferences: Mapped[Optional[dict]] = mapped_column(__import__("sqlalchemy").JSON, default=dict)
    tags: Mapped[Optional[list]] = mapped_column(__import__("sqlalchemy").JSON, default=list)
    notes: Mapped[Optional[str]] = mapped_column(Text)
    last_seen_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
    total_conversations: Mapped[Optional[int]] = mapped_column(__import__("sqlalchemy").Integer, default=0)
